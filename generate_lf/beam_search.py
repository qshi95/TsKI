...
from APIs_for_gen import APIs as APIs_for_gen
...
def dynamic_programming(name, t, mem_str, mem_num, head_str, head_num, num=6, debug=False):
    must_have = []
    must_not_have = []

    # head_str = []   # 没有数字就不计数了 这不行 但是不能保证所有statement都有数字link，为什么一定需要在表格中有数字才能count呢 正常来说 任何表格都可以count呀
    # mem_str = []
    # mem_num = []
    # head_num = []
    ...
    # mem_str: linked string entity
    # mem_num: linked number entity
    # head_str: linked string header
    # head_num: linked number header
    ...
    """
    return (name, [_[0].cur_str for _ in finished])
